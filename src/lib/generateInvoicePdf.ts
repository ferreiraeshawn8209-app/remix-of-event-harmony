...
export function generateInvoicePdf(quote: DatabaseQuote, download = true): jsPDF {
  const doc = new jsPDF();
...
export function generateQuotePdf(quote: DatabaseQuote, download = true): jsPDF {
  // Quote PDF is identical to invoice but titled "QUOTE"
  const doc = new jsPDF();
  const pageWidth = doc.internal.pageSize.getWidth();
  let y = 20;
  ...
  doc.setFontSize(18);
  doc.setFont("helvetica", "bold");
  doc.text("QUOTE", pageWidth - 20, y, { align: "right" });
  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(100);
  doc.text(`#${quote.id.slice(0, 8).toUpperCase()}`, pageWidth - 20, y + 7, { align: "right" });
  ...
  doc.setTextColor(130);
  doc.text("Thank you for choosing BeatKulture Entertainment!", pageWidth / 2, y, { align: "center" });
  doc.text("This quote is valid for 7 days from the date of issue.", pageWidth / 2, y + 5, { align: "center" });
...
export function sharePdfViaWhatsApp(quote: DatabaseQuote, type: "quote" | "invoice") {
  ...
  const doc = pdfGenerator(quote, false);
...
export function shareViaEmail(quote: DatabaseQuote, type: "quote" | "invoice") {
  const pdfGenerator = type === "quote" ? generateQuotePdf : generateInvoicePdf;
  pdfGenerator(quote, true); // downloads the file
